...
def test_gridsearch_keep_best(mcycle_X_y):
    ...
    scores = gam.gridsearch(X, y, lam=np.logspace(-3, 3, n), keep_best=False, return_scores=True)
...
def test_GCV_objective_is_for_unknown_scale(mcycle_X_y, default_X_y, coal_X_y, trees_X_y):
    ...
    unknown_scale = [(LinearGAM, mcycle_X_y), (GammaGAM, trees_X_y), (InvGaussGAM, trees_X_y)]

    known_scale = [(LogisticGAM, default_X_y), (PoissonGAM, coal_X_y)]

    for gam, (X, y) in unknown_scale:
        scores1 = list(gam().gridsearch(X, y, lam=lam, objective="auto", return_scores=True).values())
        scores2 = list(gam().gridsearch(X, y, lam=lam, objective="GCV", return_scores=True).values())
...
def test_UBRE_objective_is_for_known_scale(mcycle_X_y, default_X_y, coal_X_y, trees_X_y):
    ...
    for gam, (X, y) in known_scale:
        scores1 = list(gam().gridsearch(X, y, lam=lam, objective="auto", return_scores=True).values())
        scores2 = list(gam().gridsearch(X, y, lam=lam, objective="UBRE", return_scores=True).values())
        assert np.allclose(scores1, scores2)

    for gam, (X, y) in unknown_scale:
        try:
            list(gam().gridsearch(X, y, lam=lam, objective="UBRE", return_scores=True).values())
        except ValueError:
            assert True
